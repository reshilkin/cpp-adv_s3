...
public:
  ...
  private:
    ...
    list_iterator(S* ptr)
        : ptr_(static_cast<list_element_base*>(
              static_cast<list_element<Tag>*>(ptr))) {}
  ...
  iterator insert(iterator pos, T& e) {
    if (pos != &e) {
      auto& cur =
          static_cast<list_element_base&>(static_cast<list_element<Tag>&>(e));
      cur.unlink();
  ...
  void splice(const_iterator pos, list& other, const_iterator beg,
              const_iterator end) {
    if (beg == end) {
      return;
    }
